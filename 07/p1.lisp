...
; add or multiply the next number to the current set of results to get the next set of results
...
(defun next-step (lhss rhs)
   (let ((adding (lambda (x) (+ x rhs))) (muling (lambda (x) (* x rhs))))
      (union (mapcar adding lhss) (mapcar muling lhss))
   )
)
