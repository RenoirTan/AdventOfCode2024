...
(defun filter-nums-le-answer (answer nums)
   (remove-if #'(lambda (x) (> x answer)) nums)
)
...
(defun next-step (lhss rhs answer)
   (let* (
...
(defun next-step-safe (lhss rhs answer)
   (if (listp lhss) ; check if lhss is a list
      (next-step lhss rhs answer)
      (next-step (cons lhss nil) rhs answer)
   )
)

; get the list of possible results from list of nums
(defun possible-results (answer nums)
   (reduce #'(lambda (lhss rhs) (next-step-safe lhss rhs answer)) nums)
)

; check if answer can be obtained from list nums by addition or multiplication
(defun answer-in-result (answer nums)
   (member answer (possible-results answer nums))
)
