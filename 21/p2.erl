...
sum_human_inputs(Sequence, Previous, Sum) ->
    case Sequence of
        ...
        [Next|Rest] -> sum_human_inputs(
            Rest,
            Next,
            Sum + get_cached_length(0, 26, Previous, Next)
        )
    end.
            ...
            case Sequence of
                ...
                [NextStep|Remaining] ->
                    get_steps_at_higher_level(
                        ...
                        NextStep,
                        Steps + get_cached_length(Layer, Depth, PreviousStep, NextStep)
                    )
            end;
        true -> throw(bad_sequence)
    end.

get_cached_length(Layer, Depth, Src, Dest) ->
    case get({Layer, Depth, Src, Dest}) of
        undefined ->
            Value = get_sequence_length(Layer, Depth, Src, Dest),
            put({Layer, Depth, Src, Dest}, Value),
            Value;
        Value -> Value
    end.